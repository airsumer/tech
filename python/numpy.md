...
`numpy`是`python`科学计算中最核心、最基础的库，用于执行高性能的数值计算。它提供了一个强大的 **N维数组对象（`ndarray`）**，并在此基础上构建了大量的数学函数和操作工具。对于同样的数值计算任务，使用`numpy`比直接使用`python`要简洁高效得多。
...
typedef struct {
    ...
    npy_intp *dimensions;// 一个指针，指向一个整数数组，表示数组在每个维度上的大
    npy_intp *strides;// 一个指针，指向一个整数数组，表示访问相邻元素所需的字节偏移量
    void *data;//一个指向数组实际数据的指针
    int nd;//数组的维度
    int type_num;//表示数组元素的数据类型
    PyObject *base;//指向原始数组对象的指针，用于跟踪数组的引用
} PyArrayObject;
```
